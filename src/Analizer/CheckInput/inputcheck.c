...
int executeCommand(int type, int *n, int *m, int argc, int i, char **argv, int *recursive, int pipe_to_m[]){
	int res = 0;
	...
	else if(type == PIPEFORM && i + 2 < argc && isNumber(argv[i + 1]) && isNumber(argv[i + 2])){
		pipe_to_m[READ] = atoi(argv[i + 1]);
		pipe_to_m[WRITE] = atoi(argv[i + 2]);
		res = 2;
	}
	return res;
}
...
void readInput(int argc, char *argv[], char ***input, int *ninput, int *n, int *m, int *recursive, int pipe_to_m[]){
		(*recursive) = 0;
		int i = 1;
		int pos[argc];
		(*ninput)= 0;
		while(i < argc){
			...
			if(type == COMMAND){
				...
				int executed = executeCommand(command, n, m, argc, i, argv, recursive, pipe_to_m);
				if(executed == 1)
					i++;
				else if(executed == 2)
					i += 2;
	...
	int manageInput(char ***def_file_list, int argc, char *argv[],  int *m, int *n, int *recursive, int pipe_to_m[]){
		int nfiles, ndirs, ninput;
		char **input, **dir_list, **file_list;
		pipe_to_m[READ] = -1;
	pipe_to_m[WRITE] = -1;
	readInput(argc, argv, &input, &ninput, n, m, recursive, pipe_to_m);
